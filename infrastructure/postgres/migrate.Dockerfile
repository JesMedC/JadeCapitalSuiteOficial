...
CMD ["bash", "-c", "until pg_isready -h postgres -U $POSTGRES_USER; do sleep 2; done && psql -h postgres -U $POSTGRES_USER -d $POSTGRES_DB -v ON_ERROR_STOP=1 -f /migrations/20260806_0001_InitialIdentitySchema.sql"]
