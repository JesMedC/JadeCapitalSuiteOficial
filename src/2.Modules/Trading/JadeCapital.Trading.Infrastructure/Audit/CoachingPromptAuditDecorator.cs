...
/// <summary>
/// Per-aggregate audit decorator for <see cref="ICoachingPromptRepository"/>
/// (Wave 9, slice 9a.1 — sub-scope A coverage extension).
///
/// <para>
/// <b>Why this decorator is bespoke (does NOT use the generic
/// <c>DecoratedRepository&lt;T&gt;</c> helper)</b>: <see cref="ICoachingPromptRepository"/>
/// exposes a write-once shape — only <c>AddAsync</c> +
/// <c>FindByUserAndDateAsync</c> + <c>ListByUserAndWindowAsync</c>. The
/// <see cref="CoachingPrompt"/> aggregate is immutable after
/// <see cref="CoachingPrompt.Create"/> per the entity docstring
/// ("Immutability: the aggregate has no public setters. EF rehydration
/// uses the Rehydrate factory which is reserved for the repository and
/// skips the validation guards"). Extending <c>IRepository&lt;T&gt;</c>
/// would force <c>UpdateAsync</c> + <c>DeleteAsync</c> methods that the
/// contract forbids — breaking the write-once promise compliance officers
/// rely on for <c>entity_type = "CoachingPrompt"</c> rows in
/// <c>audit.events</c>. Bespoke decorator preserves the immutable
...
/// row's <see cref="CoachingPrompt.UserId"/> still comes from the caller.
/// Both <c>GenerateCoachingPromptHandler</c> (manual trigger) and
/// <c>CoachingPromptService</c> (daily BG service) resolve the user via
/// <c>IUserTradingContextProvider</c> + accept userId as a parameter — a
/// cross-tenant invocation could submit a prompt carrying another
/// tenant's user id. Per spec §9a.1 (the spec's Requirement "Audit
/// decorator for CoachingPrompt aggregate"): "Cross-tenant
/// <c>IsOwner</c> check MUST compare <c>prompt.UserId</c> to
/// <c>ITenantContext.CurrentUserId</c>; on mismatch, emit
/// <c>AuditAction.Denied</c> and throw <c>UnauthorizedAccessException</c>."
/// This is the only enforcement point — matches the
/// <see cref="AIRiskAdviceAuditDecorator"/> + Wave 8 8b.1 StripeCustomer
/// precedent (handler-side consistency is not guaranteed for
/// multi-entry-point mutations).
/// </para>
///
/// <para>
...
///
/// <list type="bullet">
///   <item>Wrapping <see cref="ICoachingPromptRepository.AddAsync"/> with
...
///         CoachingPrompt is the audit-relevant entity itself; the row
...
/// dependencies: inner + audit + tenant + clock), matching the Wave 8
/// 8b.1 <see cref="JadeCapital.Billing.Infrastructure.Audit.StripeCustomerAuditDecorator"/>
/// + 9a.1 <see cref="AIRiskAdviceAuditDecorator"/> minimal shape.
/// </para>
///
/// <para>
/// Registered via Scrutor:
/// <c>services.Decorate&lt;ICoachingPromptRepository, CoachingPromptAuditDecorator&gt;()</c>
/// in <see cref="DependencyInjection.TradingModuleRegistration"/>.
/// </para>
/// </summary>
public sealed class CoachingPromptAuditDecorator : ICoachingPromptRepository
{
    private readonly ICoachingPromptRepository _inner;
    private readonly IAuditLogger _audit;
    private readonly ITenantContext _tenant;
    private readonly IClock _clock;

    public CoachingPromptAuditDecorator(
        ICoachingPromptRepository inner,
        IAuditLogger audit,
    ...
    public Task<CoachingPrompt?> FindByUserAndDateAsync(
        Guid userId,
        DateTimeOffset dayUtc,
        CancellationToken ct)
        => _inner.FindByUserAndDateAsync(userId, dayUtc, ct);

    public Task<IReadOnlyList<CoachingPrompt>> ListByUserAndWindowAsync(
        Guid userId,
        DateTimeOffset from,
        DateTimeOffset to,
        CancellationToken ct)
        => _inner.ListByUserAndWindowAsync(userId, from, to, ct);
    ...
    public async Task AddAsync(CoachingPrompt prompt, CancellationToken ct)
    {
        if (!IsOwner(prompt))
        {
            ...
            // 9a.1 AIRiskAdvice precedent — Denied rows always carry the
            // rejection trail, not the would-have-been mutation).
            await LogDeniedAsync(prompt, ct);
            throw new UnauthorizedAccessException(
                $"CoachingPrompt {prompt.Id} does not belong to current user (cross-tenant attempt).");
        }

        await _inner.AddAsync(prompt, ct);
        await TryAuditAsync(BuildEntry(prompt), ct);
    }

    // ===== Helpers =====

    /// <summary>
    /// Returns true iff the CoachingPrompt belongs to the current user.
    /// When no user is resolved (anonymous / service context — e.g., the
    /// daily BG service resolving users via the trading context provider),
    ...
    /// </summary>
    private bool IsOwner(CoachingPrompt prompt)
        => !_tenant.CurrentUserId.HasValue
            || prompt.UserId == _tenant.CurrentUserId.Value;
    ...
    /// </summary>
    private Task LogDeniedAsync(CoachingPrompt prompt, CancellationToken ct)
    {
        var entry = new AuditEventEntry(
            EntityType: nameof(CoachingPrompt),
            EntityId: prompt.Id,
    ...
    /// <summary>
    /// Builds the <see cref="AuditEventEntry"/> for the created prompt.
    /// <c>EntityId</c> is the prompt's primary key; <c>EntityType</c> is
    /// the prompt type's name (so compliance officers query by
    /// <c>entity_type = "CoachingPrompt"</c> + <c>action = "Created"</c>).
    /// <c>TenantId</c> + <c>UserId</c> come from <see cref="ITenantContext"/>.
    /// Created events carry no diff (<c>ChangesJson = null</c>) — the entire
    /// prompt record is captured by the <c>audit.events</c> row's
    /// <c>entity_id</c> + the persisted <c>trading.coaching_prompts_ai</c>
    /// row.
    /// </summary>
    private AuditEventEntry BuildEntry(CoachingPrompt prompt)
        => new(
            EntityType: nameof(CoachingPrompt),
            EntityId: prompt.Id,
